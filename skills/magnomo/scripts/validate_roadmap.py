#!/usr/bin/env python3
"""Validate Magnomo roadmap.yaml and feature-map.yaml artifacts."""
...
from magnomo_utils import has_unresolved_template_token, is_missing, load_yaml_mapping, scan_unresolved_template_tokens, unique
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            if key_text in FORBIDDEN_FEATURE_MAP_KEYS:
                errors.append(f"{child_label} belongs to Mago or Magia, not Magnomo roadmap handoff")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate Magnomo roadmap artifacts.")
    ...
    if warnings:
        ...
        print("OK: validated Magnomo roadmap artifacts")
    return 0
