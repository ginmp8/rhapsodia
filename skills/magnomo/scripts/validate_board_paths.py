#!/usr/bin/env python3
"""Validate Magnomo canonical board artifact paths."""
...
from magnomo_utils import BOARD_ROOT_TEMPLATE, normalize_path, read_normalized_lines, resolve_board_root, unique

CANONICAL_PARTS = ("docs", "boards")
SLUG_RE = re.compile(r"^[a-z0-9][a-z0-9._-]*$")
SPEC_ID_RE = re.compile(r"^spec\d{3}$")
SKILL_PACKAGE_PREFIXES = (".github/skills/magnomo/", "magnomo/")
...
MAGNOMO_ARTIFACTS = BOARD_SCOPED_ARTIFACTS | SPEC_SCOPED_ARTIFACTS
...
def is_magnomo_artifact(path: str) -> bool:
    return Path(path).name in MAGNOMO_ARTIFACTS
...
def validate_path(path: str, expected_board_id: str | None, expected_cycle_version: str | None) -> list[str]:
    ...
    if not under_root and is_magnomo_artifact(path):
        return [f"{path}: Magnomo artifact must be under {BOARD_ROOT_TEMPLATE}"]
    ...
    if len(parts) == 5:
        ...
        if artifact_name not in BOARD_SCOPED_ARTIFACTS:
            if artifact_name in SPEC_SCOPED_ARTIFACTS:
                ...
                errors.append(f"{path}: artifact name is not Magnomo-owned")
        return errors
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description=f"Validate Magnomo artifact paths under {BOARD_ROOT_TEMPLATE}."
    )
