#!/usr/bin/env python3
"""Validate Magnomo markdown artifacts that rely on canonical headings and resolved placeholders."""
...
from magnomo_utils import find_unresolved_template_tokens_in_text, unique
...
def validate(path: Path) -> list[str]:
    ...
    if headings is None:
        return [f"{path}: unsupported Magnomo human artifact `{path.name}`"]
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate Magnomo human markdown artifacts.")
    parser.add_argument("paths", nargs="+", help="Path(s) to Magnomo markdown artifacts.")
