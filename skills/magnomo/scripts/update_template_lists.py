#!/usr/bin/env python3
"""Populate list fields in Magnomo template-backed artifacts with schema checks."""
...
def apply_updates(artifact_path: Path, updates: dict[str, Any]) -> None:
    ...
    if not rules:
        fail(f"unsupported Magnomo template-backed artifact `{name}`")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Populate Magnomo artifact list fields with canonical schema checks.")
    ...
    try:
        if args.schema:
            if args.artifact_name and args.artifact_name not in RULES:
                fail(f"unsupported Magnomo artifact `{args.artifact_name}`")
