...
def test_v2_valid():assert m.validate({'contract_version':2,'candidates':[row('C1'),row('C2')]})==[]
def test_evaluator_mismatch():
 d={'contract_version':2,'candidates':[row('C1'),row('C2')]};d['candidates'][1]['evaluator_id']='E2';assert 'comparability:evaluator_id:mismatch' in m.validate(d)
