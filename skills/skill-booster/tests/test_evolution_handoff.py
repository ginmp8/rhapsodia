...
def test_v1_rejected():
 d=data();d['handoff_version']=1;d['contract_version']=1
 e=m.validate(d);assert 'handoff_version:unsupported' in e and 'contract_version:unsupported' in e
def test_policy_identity_required():
 d=data();d['evaluation_identity'].pop('policy_id');assert 'evaluation_identity.policy_id:invalid' in m.validate(d)
