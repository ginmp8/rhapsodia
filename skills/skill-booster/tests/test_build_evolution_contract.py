...
def test_build_v2_contract(tmp_path):
 target='sha256:test';
 ...
 c=m.build(h,tmp_path);assert c['contract_version']==2;assert c['capability_invariants']==['inv.a'];assert c['transformation_registry'][0]['status']=='proposed';assert c['transformation_registry'][0]['addresses']==['d1']
