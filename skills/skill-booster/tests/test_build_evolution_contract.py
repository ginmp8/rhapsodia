...
def test_build_v4_contract(tmp_path):
 target='sha256:test';
 ...
 ev={'schema_version':2,'target_identity':target,'levels':[],'promotion':{},'finalist_policy':{'minimum_evaluation_level':'L4-benchmark','holdout_policy':'not-required'}}
 ...
 c=m.build(h,tmp_path);assert c['contract_version']==4;assert c['finalist_policy']==ev['finalist_policy'];assert c['capability_invariants']==['inv.a'];assert c['transformation_registry'][0]['status']=='proposed';assert c['transformation_registry'][0]['addresses']==['d1']
