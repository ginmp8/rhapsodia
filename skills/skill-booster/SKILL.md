...
Optimize one existing skill package end to end. Establish a baseline, freeze evaluators, run the ordered specialist workflow, apply bounded patches, validate after material changes, compress only after behavior is stable, harden/package only after gates pass, and separate measured evidence from judgment.
...
Default: complete optimization in target-scope only; one bounded patch batch per phase; validate after changes; package only after folder and archive checks pass.
...
- [references/evaluation-contract.md](references/evaluation-contract.md): freeze rules, metrics, acceptance, saturated-score handling.
...
3. **Specialist sequence**: run or explicitly account for every pass in this order: `skill-creator-juiced`, `skill-improver`, `skill-benchmark`, `skill-harness`, `skill-package-architecture-review`, `context-architect`, `skill-prompt-and-activation-review`, `prompt-architect`, `skill-consistency-repair`, `documentation-quality`, `karpathy-guidelines`, `security-and-governance-review`, `skill-testing-and-validation`, `skill-cleanup-and-simplification`, `skill-token-efficient`, `skill-testing-and-validation`, `skill-hardening`, final `skill-benchmark`, final `skill-improver`, final `skill-token-efficient` closure.
...
4. accepted/rejected hypotheses with files, expected effect, validation, decision, evidence;
