...
- Optimization state: for complex/full optimization, maintain capability, transformation, and evaluation artifacts outside the target worktree when material. Maintain an experiment registry only when real experiments or multi-candidate comparisons are executed. Canonical optimization must not require evolutionary metadata or artifacts.
