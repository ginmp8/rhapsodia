---
name: skill-booster
description: "use when optimizing, improving, benchmarking, hardening, compressing, validating, normalizing portability, or packaging an existing Agent Skills-compatible or SKILL.md-based skill, including third-party skills downloaded from the internet, across ChatGPT/OpenAI, Codex, Claude, GitHub Copilot, Cursor, or another compatible host; runs trust intake for external skills, evidence-driven specialist routing, portability normalization through Skill Creator Juiced, optional reproducibility engineering, bounded changes, validation, final freeze, atomic packaging, and truthful readiness reporting; do not use for net-new skill creation, generic repository refactors, or unsupported measured-improvement claims"
---
...
1. **Preflight, trust, host capabilities, and inventory**: confirm one root `SKILL.md`; classify `SOURCE_CLASS`; read [references/external-skill-intake.md](references/external-skill-intake.md) for `external-untrusted-skill` before executing any target-owned script. When external, run `<PYTHON> scripts/inspect_external_skill.py --target <TARGET_OR_ARCHIVE> --json <WORK>/external-intake.json`; resolve blocking findings before target execution and preserve the receipt. Then read [references/host-compatibility.md](references/host-compatibility.md). Resolve `TARGET_HOSTS`, writable filesystem/process capabilities, specialist-dispatch capability, and `PYTHON`. Run the Booster-owned structural validator `<PYTHON> scripts/validate_skill_booster.py --target <TARGET_SKILL_PATH>` without executing target code. Always validate `portable-core` for mutating optimization. For `complete optimization`, run `<PYTHON> scripts/validate_portability.py --target <TARGET_SKILL_PATH> --hosts portable-core,openai,codex,claude,copilot,cursor` unless the user explicitly narrowed the matrix. Inventory core dirs, optional host adapters, validators, packages, dependencies, executable files, and generated files. Do not treat `agents/openai.yaml`, Cursor-only frontmatter, Claude/Codex install paths, Copilot paths, or any vendor-private API as part of the portable core.
...
6. **Portability closure, package, and close**: rerun portability against the frozen candidate. `portable-core` is a hard gate for every mutating optimization. `complete optimization` additionally requires an explicit result for `openai`, `codex`, `claude`, `copilot`, and `cursor`; a host may be `validated`, `structurally-compatible`, `adapter-required`, `capability-limited`, `not-run`, or `unsupported`, but structural evidence must never be mislabeled runtime validation. Immediately before packaging, verify the frozen candidate with `<PYTHON> scripts/freeze_candidate.py verify --target <TARGET_SKILL_PATH> --manifest <WORK>/candidate-manifest.json`. Package only when verification, validation, source-identity checks when applicable, and required specialist reconciliation pass. For complete optimization use `<PYTHON> scripts/package_skill.py --target <TARGET_SKILL_PATH> --output <OUTPUT_DIR>/skill.zip --report <REPORT_PATH> --portability-hosts portable-core,openai,codex,claude,copilot,cursor`; add `--reconciliation-ledger <LEDGER_JSON>` when required. Preflight package/report paths before mutation. The packager must commit archive+success receipt transactionally, preserve last-known-good archive/receipt on failure, preserve recovery evidence if rollback is incomplete, and bind hashes plus portability results to the exact delivered archive.
