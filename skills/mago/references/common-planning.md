...
Load defaults from [canonical-paths.md](canonical-paths.md). `BOARD_ROOT` is the active boundary; use prompt-provided root when valid, otherwise derive it from concrete `board_id` and `cycle_version`. Package paths derive from `BOARD_ROOT/specs/<spec_id>/`. Do not create parallel aliases.
...
- manifest.yaml: identity, classification, `status`, `phase`, `source_of_truth`, traceability, optional truthful `last_execution`.
- prd.md, technical-design.md, notes.md, validation.md, tasks.md when present: detailed planning meaning.
...
- Product-only: `define-product` / `refine-product` touch only prd.md, notes.md, optional validation.md; do not alter tasks.md, catalog order, or execution state. Product-only notes omit `Specialist Rationale` and `Execution Log` unless truthful existing sections must stay.
...
- Technical-design: touch technical-design.md only, except bounded consistency fixes to notes.md or validation.md justified by the same evidence. Use for architecture, contracts, migration, observability, security, rollback, or option alignment. Do not create task decomposition, repo changes, execution steps, or validation evidence. If evidence is thin, write a small design with unknowns/open questions. Size: `small`, `medium`, `large`, or `unknown`. Verify facts through package evidence, repository code/tests/config/docs, or official dependency docs; unresolved claims go to `Open Questions`.
...
MAGO plans/refines/documents; it does not implement product code or run execution workflows. Execution headings such as `Execution Rules`, `Execution Log`, task `Validation`, or `phase: execute` are artifact schema. Reflect execution progress only when repository evidence or existing planning truth supports it. If a request crosses into implementation, stop at planning.
...
- notes.md `Execution Log` statuses: `not_started`, `in_progress`, `blocked`, `done`.
- `manifest.yaml.last_execution`: required `task_id`; optional `date`, `summary`, `files_changed`; preserve truthful legacy extra keys, do not invent noncanonical keys; omit when no task has truthfully executed.
- If task ids split or new ids appear, preserve old truthful history under old ids and add new execution-log subsections only for new ids.
