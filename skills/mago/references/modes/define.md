...
- manifest.yaml: required `kind`, `spec_id`, `cycle_id`, `feature_key`, `title`, `type`, `classification`, `status`, `phase`, `feature_version`, `created_at`, `source_of_truth`, `traceability`; optional truthful `last_execution`; planning-only usually `status: planned`, `phase: define`; keep `source_of_truth` paths lowercase and discovery traceability when applicable.
