...
Use `references/architecture-decisions.md` and `references/adr-quality.md` for ADR content and quality. Do not create architecture ADRs in nomia.

## Evidence and Optional Sections

Design facts must be traceable to the selected package, repository truth, local docs, supplied nomia handoff evidence, or official dependency documentation. When the chain does not establish a fact, use `unknown` prose and add an `Open Questions` item.
