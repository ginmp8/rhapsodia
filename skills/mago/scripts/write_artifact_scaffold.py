#!/usr/bin/env python3
"""Write MAGO artifacts from canonical templates instead of freehand copying."""
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Write a MAGO artifact scaffold from the canonical template workflow.")
    parser.add_argument("path", help="Destination artifact path.")
    parser.add_argument("--template", help="Explicit template file name, for example discovery-candidate.md.template.")
    ...
    if not template_name:
        print(f"ERROR: could not infer a MAGO template for `{destination.name}`")
