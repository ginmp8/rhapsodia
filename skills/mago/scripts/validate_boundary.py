...
DIRECT_DEPENDENCY_PATTERNS = [
    (".github" + "/skills/" + MAGNOMO, "references the sibling skill directory"),
    (".github" + "\\skills\\" + MAGNOMO, "references the sibling skill directory"),
    ("skills://" + MAGNOMO, "uses another skill URI"),
    ("../" + MAGNOMO, "uses a relative path to another skill"),
    ("..\\" + MAGNOMO, "uses a relative path to another skill"),
]
...
PY_IMPORT_RE = re.compile(r"\b(?:from|import)\s+[.\w]*" + MAGNOMO + r"\b", re.IGNORECASE)
    ...
    if path.suffix == ".py" and PY_IMPORT_RE.search(text):
        for line_number, line in enumerate(text.splitlines(), start=1):
            if PY_IMPORT_RE.search(line):
                errors.append(f"{posix_rel(path, root)}:{line_number}: imports a module named `{MAGNOMO}`")
...
def validate_script_artifact_targets(root: Path) -> list[str]:
    ...
    for path in sorted(scripts_root.glob("*.py")):
        ...
        for artifact in MAGNOMO_OWNED_ARTIFACTS:
