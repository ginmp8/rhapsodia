#!/usr/bin/env python3
"""Validate Magiarca contracts and actor write boundaries."""
...
from magiarca_utils import load_yaml_mapping, read_normalized_lines, unique
...
MAGIARCA_ALLOWED_CODE_DIR = ".github/skills/magiarca/scripts/"
CONTRACT_YAML_REQUIREMENT = "PyYAML is required to validate Magiarca contract YAML artifacts."
CODE_SUFFIXES = {
    ".py",
    ".js",
    ".jsx",
...
def is_magiarca_owned(path: str) -> bool:
    name = Path(path).name
    return is_under_skill(path, "magiarca") or name in MAGIARCA_ARTIFACTS or "/magiarca/" in path
...
def is_repository_code(path: str) -> bool:
    if path.startswith(MAGIARCA_ALLOWED_CODE_DIR) or is_under_skill(path, "magiarca"):
        return False
...
def validate_actor(actor: str, changed_files: list[str]) -> list[str]:
    ...
    for path in changed_files:
        name = Path(path).name
        if actor == "mago":
            if is_magiarca_owned(path):
                errors.append(f"mago must not write Magiarca-owned artifact `{path}`")
            ...
            if is_magiarca_owned(path):
                errors.append(f"magia must not write Magiarca-owned artifact `{path}`")
            ...
            if is_under_skill(path, "mago") or is_under_skill(path, "magia"):
                errors.append(f"magiarca must not write Mago or Magia files: `{path}`")
            elif is_mago_owned(path):
                errors.append(f"magiarca must not write Mago planning package artifact `{path}`")
            elif is_magia_owned(path):
                errors.append(f"magiarca must not write Magia execution record `{path}`")
            elif is_repository_code(path):
                errors.append(f"magiarca must not write repository code or execution paths: `{path}`")
        else:
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate Magiarca contracts.")
    parser.add_argument("--actor", choices=["mago", "magia", "magiarca"], help="Actor that produced the changed files.")
    ...
    print("OK: validated Magiarca contracts")
    return 0
