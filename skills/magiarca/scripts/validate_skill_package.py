#!/usr/bin/env python3
"""Validate Magiarca skill package structure before packaging."""
...
def validate_skill_md(root: Path, errors: list[str]) -> None:
    ...
    if frontmatter.get("name") != "magiarca":
        errors.append("SKILL.md name must be magiarca")
...
def validate_harness_scenarios(root: Path, errors: list[str]) -> None:
    ...
    if not isinstance(data, dict):
        ...
        return
    if data.get("target_skill") != "magiarca":
        errors.append("harness scenarios target_skill must be magiarca")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Magiarca skill package structure before packaging.")
    parser.add_argument("--target", default=str(Path(__file__).resolve().parents[1]), help="Magiarca skill root. Defaults to this script's parent skill.")
    ...
    if errors:
        ...
        print("all Magiarca skill package checks passed")
