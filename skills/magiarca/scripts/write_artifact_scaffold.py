#!/usr/bin/env python3
"""Write Magiarca artifacts from canonical template scripts instead of freehand template copying."""
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Write a Magiarca artifact scaffold from the canonical template workflow.")
    ...
    if not template_name:
        print(f"ERROR: could not infer a Magiarca template for `{destination.name}`")
