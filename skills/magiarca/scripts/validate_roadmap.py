#!/usr/bin/env python3
"""Validate Magiarca roadmap.yaml and feature-map.yaml artifacts."""
...
from magiarca_utils import has_unresolved_template_token, is_missing, load_yaml_mapping, scan_unresolved_template_tokens, unique
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            if key_text in FORBIDDEN_FEATURE_MAP_KEYS:
                errors.append(f"{child_label} belongs to Mago or Magia, not Magiarca roadmap handoff")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate Magiarca roadmap artifacts.")
    ...
    if warnings:
        ...
        print("OK: validated Magiarca roadmap artifacts")
    return 0
