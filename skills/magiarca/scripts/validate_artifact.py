#!/usr/bin/env python3
"""Dispatch Magiarca artifact validation without relying on manual validator selection."""
...
def validate_one(path: Path) -> int:
    ...
    print(f"ERROR: unsupported Magiarca artifact `{name}`")
    return 1


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate a Magiarca artifact with the canonical validator for its template family.")
