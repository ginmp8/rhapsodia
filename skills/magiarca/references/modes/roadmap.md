...
Do: generate handoff entries with `feature_key`, `candidate_spec_id`, `title`, `scope_summary`, `dependencies`, and `recommended_mago_mode`; use source context from Magiarca roadmap artifacts; recommend Mago mode such as `define`, `refine`, or `split` when clear; mark unresolved candidates `ready_for_spec: false` or `candidate_spec_id: null`.
