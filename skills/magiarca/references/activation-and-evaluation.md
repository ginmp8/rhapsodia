...
Use for Magiarca activation decisions, `examples/activation-scenarios.json`, or scenario coverage before packaging.

## Activation Decision

Activate for create/update/normalize/validate/report requests on Magiarca-owned governance artifacts: delivery metadata, ops/status/stakeholder/replanning records, portfolio, roadmap, feature maps, RFC proposals, governance decision logs, feature reports, release notes, internal notes, readiness, and contract validation.
...
2. Classify owner: Magiarca = governance/readiness; Mago = planning/spec/task decomposition; Magia = execution/repository work.
3. Split mixed requests: do the Magiarca part; mark Mago, Magia, implementation, deployment, source-control, test, runner, branch, commit, and PR parts untouched.
...
5. Preserve owners, dates, deployment/review/validation/release facts as unknown unless supplied by user evidence or existing Magiarca artifacts.
...
```bash
python .github/skills/magiarca/scripts/validate_activation_scenarios.py .github/skills/magiarca/examples/activation-scenarios.json
python .github/skills/magiarca/scripts/validate_skill_package.py --target .github/skills/magiarca
python .github/skills/magiarca/scripts/validate_golden_examples.py --skill-root .github/skills/magiarca
```

Prefer `scripts/package_skill.py --target .github/skills/magiarca --output <output-dir>/skill.zip`; it reruns structural, activation, and golden gates before writing.
