...
Do not store branches, PRs, commits, checks, review state, deployments, or last commit age as maintained nomia status. Do not create Mago tasks, execution-handoff plans, technical designs, code changes, tests, or execution evidence. Use such material only as linked evidence when supplied.
