...
Exactly one mode selected; `BOARD_ROOT`, `board_id`, `year`, `cycle_id`, and required `spec_id` resolved before repository-facing writes; canonical ids validated, externally sourced provenance recorded, and ids not invented, selected, corrected, renamed, or registered by nomia; no registry record, generated catalog, or generated queue written; no Mago/Magia artifacts, implementation/deployment/test/runner files, branch/commit/PR records, architecture ADRs, technical designs, or implementation tasks modified; template artifacts use bundled scripts; unknown/volatile facts are not invented; touched artifacts pass validators; repository-facing writes pass board-path validation; scenario edits preserve categories and pass `scripts/validate_activation_scenarios.py` or `scripts/validate_skill_package.py`; structural edits pass `scripts/validate_skill_package.py`; golden-sensitive edits pass `scripts/validate_golden_examples.py`; identity/path changes pass `scripts/validate_identity_contract.py`; every package run passes `scripts/validate_contract_preservation.py` and the unit tests under `tests/`; `skill.zip` is produced only from passing external validation evidence for the exact target-tree digest; the packager must run from a trusted protected source, execute no target validators, reject symlinks, and exclude caches, generated evidence/reports, secrets, credentials, and old zips.
