...
def local_surface_errors(root: Path) -> list[str]:
    ...
    if skill == "mago":
        ...
        validator = (root / "scripts/validate_ops.py").read_text(encoding="utf-8")
        if '"priority" in data' not in validator or "unsupported generic key `priority`" not in validator:
            errors.append("Nomia validator must hard-reject generic priority")
