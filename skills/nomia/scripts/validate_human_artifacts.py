#!/usr/bin/env python3
"""Validate nomia markdown artifacts that rely on canonical headings and resolved placeholders."""
...
from nomia_utils import find_unresolved_template_tokens_in_text, unique
...
def validate(path: Path) -> list[str]:
    ...
    if headings is None:
        return [f"{path}: unsupported nomia human artifact `{path.name}`"]
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate nomia human markdown artifacts.")
    parser.add_argument("paths", nargs="+", help="Path(s) to nomia markdown artifacts.")
