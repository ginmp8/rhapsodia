#!/usr/bin/env python3
"""Dispatch nomia artifact validation without relying on manual validator selection."""
...
def validate_one(path: Path) -> int:
    ...
    print(f"ERROR: unsupported nomia artifact `{name}`")
    return 1


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate a nomia artifact with the canonical validator for its template family.")
