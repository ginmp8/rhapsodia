...
REQUIRED_SKILL_PHRASES = {
    "mago": ["ecosystem handoff contract", "scripts/ecosystem_handoff.py", "mago_to_magia", "mago_to_nomia", "nomia_to_mago", "magia_to_mago"],
    "magia": ["ecosystem handoff contract", "scripts/ecosystem_handoff.py", "mago_to_magia", "magia_to_mago", "magia_to_nomia"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        ...
        return 1
    digest = hashlib.sha256((root / CONTRACT_FILE).read_bytes()).hexdigest()
    print(f"OK: ecosystem handoff contract validated for {package_role(root)}")
    if args.print_hash: print(f"sha256: {digest}")
    return 0
