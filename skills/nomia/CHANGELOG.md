...
- Preserve Nomia as governance/product/delivery owner only while strengthening closure separation, deterministic route identity, package receipts, and handoff-ledger last-known-good recovery/receipts.
- Add explicit ownership contract and coordinated migration path without peer runtime reads/imports.
- Compatibility impact: compatible coordinated minor; governance authority is unchanged, handoff schema remains `3.0.0`, priority schema remains `2.0.0`, and exact-version policy requires all three 1.10.0 packages together.
