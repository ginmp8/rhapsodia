...
contract_mod = load("contract_v4", SCRIPTS / "validate_search_contract.py")
state_mod = load("state_v4", SCRIPTS / "validate_search_state.py")
...
def test_contract_rejects_v3_instead_of_silently_reinterpreting():
    c = template("search-contract.json.template")
    c["contract_version"] = 3
    assert "contract_version:upgrade_required_v4" in contract_mod.validate(c)
