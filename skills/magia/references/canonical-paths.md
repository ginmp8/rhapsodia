...
- For spec-scoped execution, derive the selected spec package path from `BOARD_ROOT/specs/<spec_id>/`.
- Treat every other canonical file path as derived from `BOARD_ROOT`.

## Required Dynamic Inputs

- `board_id` and `cycle_version` are required to derive canonical `BOARD_ROOT`.
- `spec_id` is required when the run targets one selected spec package.
