...
10. When task completion or records changed, use `scripts/close_execution_state.py <board_root> --spec-id <spec_id> --task-id <taskNNN> --status <in_progress|blocked|done> ...`; it may self-heal narrow drift. Fall back to `scripts/sync_execution_state.py <board_root> --spec-id <spec_id> ...` plus `scripts/validate_execution_state.py <board_root> --spec-id <spec_id>` only when wrapper is unavailable.
