...
- Do not mark tasks done, change manifest/spec-registry status, or set `last_execution` unless current converted evidence is complete enough and the execution-state validators pass.
...
Use `scripts/adapt_legacy_execution_records.py <board_root> --spec-id <spec_id>` when available. The script performs best-effort extraction from legacy files and writes current MAGIA-owned artifacts without deleting MAGO-owned inputs.
