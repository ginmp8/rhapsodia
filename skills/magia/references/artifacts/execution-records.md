...
- `registry/<spec_id>.yaml`: preserve `kind`, `spec_id`, `cycle_id`, `feature_key`, `feature_version`, `title`, `type`, `classification`, `created_at`, read-only `business_priority`, `technical_criticality`, `execution_sequence`, dependency fields, supersession fields, handoff, and import provenance. MAGIA may update only evidence-backed technical execution `status`.
