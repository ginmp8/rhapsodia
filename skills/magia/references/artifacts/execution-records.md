...
- `registry/<spec_id>.yaml`: preserve `kind`, `spec_id`, `spec_uid`, `cycle_id`, `feature_key`, `feature_version`, `title`, `type`, `classification`, `created_at`, `priority`, `order_hint`, dependency fields, supersession fields, handoff, and import provenance. MAGIA may update only evidence-backed technical execution `status`.
