...
Every MAGIA technical artifact includes scope/task/spec link when available, evidence inspected, actual decision or implementation fact, validation status with skipped checks and reasons, residual risks/unknowns, and handoff decision `none`, `mago`, `nomia`, or `both`.
...
- nomia: delivery risk, deadline change, stakeholder communication, release-note need, owner change, or accepted business risk.
