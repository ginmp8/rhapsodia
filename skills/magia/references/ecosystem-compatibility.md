...
Release-time acceptance also requires `references/ecosystem-reproducibility-contract.json`, `references/ecosystem-ownership-contract.json`, the frozen cross-skill suite, package receipts, and an ecosystem compatibility receipt. These remain byte-equivalent local copies/evidence and do not introduce peer-runtime dependency.
