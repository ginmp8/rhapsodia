...
def local_errors(root):
 ...
 return out
def validate(root):
 err=[]; version=(root/'VERSION').read_text().strip(); contract=load(root/'references/ecosystem-routing-contract.json'); corpus=load(root/'evals/ecosystem-routing-scenarios.json'); compatibility=load(root/'references/ecosystem-compatibility.json'); repro=load(root/'references/ecosystem-reproducibility-contract.json')
