...
def collect_errors() -> list[str]:
    ...
    for path in iter_skill_files():
        ...
        # The shared ownership contract must enumerate peer-owned artifacts/modes as
        # normative read-only evidence. Exempt only that exact contract from the
        # lexical ownership-reference scan; hard-coupling and invalid-claim scans
        # still apply, and the contract is independently validated cross-skill.
        normative_ownership_contract = rel(path) == "references/ecosystem-ownership-contract.json"
        if not normative_ownership_contract:
