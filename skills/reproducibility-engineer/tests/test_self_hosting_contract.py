...
def run(data: dict) -> subprocess.CompletedProcess[str]:
    with tempfile.TemporaryDirectory() as td:
        ...
        return subprocess.run([sys.executable, str(SCRIPT), str(path), "--strict"], capture_output=True, text=True)


class SelfHostingContractTests(unittest.TestCase):
    def test_self_hosting_contract_passes(self) -> None:
        result = run(contract())
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)

    def test_same_controller_candidate_fails(self) -> None:
        data = contract()
        data["self_hosting"]["candidate_identity"] = "controller"
        result = run(data)
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("must differ", result.stdout)

    def test_external_promotion_required(self) -> None:
        data = contract()
        data["self_hosting"]["promotion_external_to_candidate"] = False
        result = run(data)
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("promotion_external_to_candidate", result.stdout)
