...
- [`references/multi-candidate-execution-strict.md`](references/multi-candidate-execution-strict.md): additive strict contract for identity-bound repeated candidate runs and trace-manifest provenance; use only when the caller accepts the strict contract or a coordinated consumer migration exists.
...
- `contracts/integration-manifest.json`: declares the unchanged v2 compatibility surface plus the additive strict multi-candidate evidence surface for cross-skill impact analysis.

For evolutionary/search evidence, do not claim candidate-byte uniqueness across logical candidates or trace-manifest provenance from the compatibility v2 envelope alone. When those guarantees are required, the caller must accept `skill-opt.harness-multi-candidate-evidence-strict` v1 and its validator must pass. If the current consumer accepts only v2, report strict provenance as `not-proven` rather than silently tightening v2.
